...
class OddsApiClient:
    ...
    def __init__(self, api_key, regions, markets):
        ...
        for item in payload:
            ...
            if item.get('id') and item.get('commence_time'):
                events.append(
                    Event(
                        str(item['id']),
                        str(item.get('sport_key', sport_key)),
                        str(item.get('sport_title', sport_key)),
                        datetime.fromisoformat(item['commence_time'].replace('Z', '+00:00')),
